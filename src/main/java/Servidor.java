...
public class Servidor{
    ...
    public static void main(String[] args) throws Exception {
        ...
        s.createContext("/envio", Servidor::envio);       // curtir / não curtir
    ...
    private static void professor(HttpExchange t) throws IOException {
        ...
        String senha = pega(c, "senha");
        // Extrai os valores do banco de dados que foram decodificados e coloca eles em seus respectivos campos

        try (PreparedStatement ps = con.prepareStatement(
                "INSERT INTO dados (nome, descricao, senha, envio) VALUES (?,?,?,?)")) { // Há pontos de interrogação porque a ordem (ainda) é desconhecida

            ps.setString(1, nome);
            ps.setString(2, desc); // type: text
            ps.setString(3, senha); // organizando os valores em seus respectivos parâmetros (type: data)
    ...
    private static void aluno(HttpExchange t) throws IOException {
        ...
        html.append("<h1>Olá, aluno!</h1>");
        html.append("<p>Suas atividades estão aqui:</p>"); // Funciona de forma semelhante ao HTML comum

        try (Statement st = con.createStatement();
             ResultSet rs = st.executeQuery("SELECT id, nome, descricao, senha, envio FROM dados ORDER BY id DESC")) { // para mostrar os dados obtidos
            ...
            while (rs.next()) {
                ...
                String classeExtra = "";
                if ("enviar".equals(envio)) { // Se a postagem for curtida, mostre que o card foi curtido
                    classeExtra = "Atividade Enviada com sucesso!";
                } else if ("nao".equals(envio)) {
                    classeExtra = "Atividade não enviada";
                }
                ...
                html.append("<p><strong>Status:</strong> ").append(envio).append("</p>");
                ...
                html.append("<input type=\"hidden\" name=\"acao\" value=\"envio\">");
                html.append("<button type=\"submit\">Enviar</button>");
                html.append("</form>");
                ...
                html.append("<button type=\"submit\">Não enviar</button>");
                html.append("</form>");
    ...
    private static void envio(HttpExchange t) throws IOException {
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(
                    "UPDATE dados SET envio = ? WHERE id = ?")) {
                ps.setString(1, acao);
                ps.setInt(2, id);
                ps.executeUpdate();
            }
